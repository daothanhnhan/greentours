...
class Pagination
{
    ...
    private function __link1($page)
    {
        ...
        $page--;
         if ($this->_config['search'] == '') {
            return '/search-tour/'.$page;
        } else {
            return '/search-tour/'.$page.'/'.$this->_config['search'];
        }
    ...
    private function __link3($page)
    {
        ...
        $page--;
         if ($this->_config['search'] == '') {
            return '/search-service/'.$page;
        } else {
            return '/search-service/'.$page.'/'.$this->_config['search'];
        }
        
    }

    private function __link4($page)
    {
        // Nếu trang < 1 thì ta sẽ lấy link first
        if ($page <= 1 && $this->_config['link_first']){
            return $this->_config['link_first'];
    ...
    {   
        ...
        if ($this->_config['total_record'] > $this->_config['limit'])
        {
            ...
            if ($this->_config['current_page'] > 1)
            {
                $p .= '<li><a href="'.$this->__link4('1').'">First</a></li>';
                $p .= '<li><a href="'.$this->__link4($this->_config['current_page']-1).'">Prev</a></li>';
            }
            ...
            for ($i = $this->_config['min']; $i <= $this->_config['max']; $i++)
            {
                ...
                if ($this->_config['current_page'] == $i){
                    ...
                    $p .= '<li><a href="'.$this->__link4($i).'">'.$i.'</a></li>';
                }
            }
            ...
            if ($this->_config['current_page'] < $this->_config['total_page'])
            {
                $p .= '<li><a href="'.$this->__link4($this->_config['current_page'] + 1).'">Next</a></li>';
                $p .= '<li><a href="'.$this->__link4($this->_config['total_page']).'">Last</a></li>';
            }
