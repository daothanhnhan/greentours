...
public function getProductTag_url ($lang, $url) {
    global $conn_vn;
    $sql = "SELECT * FROM producttag_languages WHERE languages_code = '$lang' AND friendly_url = '$url'";
    $result = mysqli_query($conn_vn, $sql);
    $row = mysqli_fetch_assoc($result);
    return $row;
}
...
public function search_spec ($trang, $limit) {
    global $conn_vn;
    $where = "WHERE 1=1";
    $having = "";
    $cond = "";
    if (!empty($_GET['duration'])) {
        ...
        foreach ($_GET['duration'] as $item) {
            ...
            if ($item == 1) {
                $having .= " count BETWEEN 0 and 5";
            }
            if ($item == 2) {
                $having .= " count BETWEEN 6 and 10";
            }
            if ($item == 3) {
                $having .= " count BETWEEN 11 and 15";
            }
            if ($item == 4) {
                $having .= " count > 15";
            }
    ...
    if (!empty($_GET['destination']) || !empty($_GET['theme'])) {
        $where .= " AND (1=0";
        foreach ($_GET['destination'] as $item) {
            $where .= " OR product.productcat_ar LIKE '%$item%'";
            $cond .= "&destination[]=$item";
        }
    ...
    $sql = "SELECT count(itinerary.product_id) as count, product.productcat_ar as productcat_arr, product.product_id as product_id FROM product INNER JOIN itinerary ON product.product_id = itinerary.product_id $where GROUP BY product.product_id $having ORDER BY product.product_id DESC";
    $result = mysqli_query($conn_vn, $sql);
    // echo $sql;
    $count = mysqli_num_rows($result);

    $start = $start = ($trang-1) * $limit;
    $sql = "SELECT count(itinerary.product_id) as count, product.productcat_ar as productcat_arr, product.product_id as product_id FROM product INNER JOIN itinerary ON product.product_id = itinerary.product_id $where GROUP BY product.product_id $having ORDER BY product.product_id DESC LIMIT $start,$limit";
    $result = mysqli_query($conn_vn, $sql);
    $rows = array();
    while ($row = mysqli_fetch_assoc($result)) {
        $rows[] = $row;
    ...
    $config = array(
                        ...
                        'limit'         => $limit,// limit
                        'start'         => 0, // start
                        'link_full'     => 'index.php?page=search-spec'.$cond.'&trang={trang}',// Link full có dạng như sau: domain/com/page/{page}
                        'link_first'    => 'index.php?page=search-spec'.$cond,// Link trang đầu tiên
