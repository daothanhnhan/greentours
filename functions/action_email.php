        ...
        function lien_he1 () {
            global $conn_vn;
                if (isset($_POST['lien_he'])) {
                        ...
                        $sql = "INSERT INTO lien_he (firstname, lastname, email, phone, address, comment, company) VALUES ('$firstname', '$lastname','$email','$phone','$address','$note', '$company')";
                        $result = mysqli_query($conn_vn, $sql);
                        // $str = $this->form($name, $phone, $email, $note);
                        //echo $str;die;
                        // $this->email_send($email, $name, $str);
                        if ($result) {
                            echo '<script type="text/javascript">alert(\'Register contact success.\');</script>';
                        } else {
