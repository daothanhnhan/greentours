<?php 
	...
	function khoi_hanh ($id) {
		...
		if (isset($_POST['add_trademark'])) {
			...
			$sql = "UPDATE khoi_hanh SET name = '$name', ngay_di = '$ngay_di', ngay_den = '$ngay_den', price = $price, giam = '$giam' WHERE id = $id";//echo $sql;
			$result = mysqli_query($conn_vn, $sql);
			if ($result) {
				echo '<script type="text/javascript">alert(\'Bạn đã sửa được một Khởi hành.\')</script>';
			} else {
	...
	khoi_hanh($_GET['id']);
    $info = $action->getDetail('khoi_hanh', 'id', $_GET['id']);
    $giam = json_decode($info['giam'], true);
...
<form action="" method="post" enctype="multipart/form-data">
    <div class="rowNodeContentPage">
        <div class="leftNCP">
            ...
            <p class="subLeftNCP"><a href="index.php?page=khoi-hanh&producttag_id=<?= $info['producttag_id'] ?>">Quay lại</a><br /><br /></p>     
        ...
        <div class="boxNodeContentPage">
            ...
            <input type="date" class="txtNCP1" name="ngay_di" value="<?= $info['ngay_di'] ?>" required/>
            <p class="titleRightNCP">Ngày đến</p>
            <input type="date" class="txtNCP1" name="ngay_den" value="<?= $info['ngay_den'] ?>" required/>
        ...
        <div class="boxNodeContentPage">
            ...
            <div class="rowNCP">
                ...
                <div class="subColContent">
                    ...
                    <div id="size">
                        <?php foreach ($giam as $item) { ?>
                        <div class="del-input"><input type="text" class="txtNCP1" value="<?= $item ?>" name="giam[]"/><button type="button" onclick="del_size(this)">Xóa</button></div>
                        <?php } ?>
                    </div>                    
