	...
	function hanh_trinh () {
		...
		if (isset($_POST['add_hanh_trinh'])) {
			...
			try {
				...
				$sql = "INSERT INTO itinerary (name, note, product_id) VALUES (?, ?, $product_id)";
				$stmt = $conn_vn->prepare($sql);
				$stmt->bind_param("ss", $name, $note);
				$stmt->execute();

				echo '<script type="text/javascript">alert(\'Bạn đã thêm được một hành trình.\');window.location.href="index.php?page=hanh-trinh&product_id='.$product_id.'"</script>';
			}
			catch (Exception $e) {
...
<form action="" method="post" enctype="multipart/form-data">
    <div class="rowNodeContentPage">
        <div class="leftNCP">
            ...
            <p class="subLeftNCP"><a href="index.php?page=hanh-trinh&product_id=<?= $_GET['product_id'] ?>">Quay lại</a><br /><br /></p>     
